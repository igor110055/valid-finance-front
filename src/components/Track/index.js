    ...
    return (
        <Box sx={{ display: 'grid' }}>
            {isLoading ?
                ...
                :
                <Box sx={{ width: 'auto !important', borderRadius: '10px', padding: '10px 10px', margin: '0px', display: 'flex' }}>
                    <Paper sx={{ width: '100%' }}>
                        <TableContainer>
                            <Table className='currency-table' stickyHeader aria-label="sticky table">
                                <TableHead>
                                    <TableRow>
                                        {columns.map((column) => (
                                            <TableCell
                                ...
                                <TableBody className='currency-table-body'>
                                    {crypto
                                                        ...
                                                        if (column.id === 'chart') {
                                                            return (
                                                                <TableCell key={column.id + i} align={column.align} style={{ borderBottom: 'gray', color: '#ff4a68' }}>
                                                                    <img alt='img' src={`https://s2.coinmarketcap.com/static/img/coins/64x64/${value}.png`} style={{ borderRadius: '50%', width: '30px' }} />
                                                                </TableCell>
                                                            )
                                                        } else {
                                                            return (
